...
const INTRO_ITEMS = [
  {
    ...
    titleKey: "intro.item1Title",
    descKey: "intro.item1Desc",
  },
  {
    icon: Lock,
    colorClass: "text-purple-500",
    titleKey: "intro.item2Title",
    descKey: "intro.item2Desc",
  },
  {
    icon: Globe,
    colorClass: "text-green-500",
    titleKey: "intro.item3Title",
    descKey: "intro.item3Desc",
  },
  {
    icon: Filter,
    colorClass: "text-orange-500",
    titleKey: "intro.item4Title",
    descKey: "intro.item4Desc",
  },
  {
    icon: BarChart3,
    colorClass: "text-red-500",
    titleKey: "intro.item5Title",
    descKey: "intro.item5Desc",
  },
  {
    icon: Zap,
    colorClass: "text-yellow-500",
    titleKey: "intro.item6Title",
    descKey: "intro.item6Desc",
  },
  {
    icon: Cpu,
    colorClass: "text-cyan-500",
    titleKey: "intro.item7Title",
    descKey: "intro.item7Desc",
  },
  {
    icon: Edit3,
    colorClass: "text-pink-500",
    titleKey: "intro.item8Title",
    descKey: "intro.item8Desc",
  },
];
  ...
  return (
    ...
    >
      {displayItems.map((item, idx) => {
        ...
        const displayIdx = ((idx % INTRO_ITEMS.length) + 1)
          .toString()
          .padStart(2, "0");
          ...
          >
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-between">
                <span
                  className={`font-mono text-xs font-bold tracking-widest ${isActive ? "text-blue-600" : "text-gray-400"}`}
                >
                  /{displayIdx}
                </span>
                <div
                  className={`transition-transform duration-700 ${isActive ? "rotate-0 scale-110" : "rotate-12 opacity-50"}`}
                >
...
export const AuthView: React.FC<AuthViewProps> = ({ onSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isLogin && !/^[a-zA-Z0-9]{5,15}$/.test(username)) {
      setError(t("auth.formatErrorUsername"));
      return;
    }
    ...
    if (
      ...
      setError(t("auth.formatErrorPassword"));
      return;
    }
  ...
  return (
    <div className="flex flex-row min-h-screen bg-white dark:bg-gray-950 overflow-hidden relative">
      ...
      {/* 移动端全屏背景 - 点击可唤回面板 */}
      <div 
        className={`lg:hidden absolute inset-0 z-0 transition-opacity duration-500 cursor-pointer ${isPanelVisible ? "opacity-25" : "opacity-70"}`}
      ...
      >
        ...
        <div className="absolute inset-0 overflow-hidden opacity-20 pointer-events-none lg:hidden">
        ...
        <Card
          ...
          onClick={(e) => e.stopPropagation()} // 防止点击卡片内部触发父容器的收起逻辑
        >
          <div className="flex flex-col items-center mb-8">
            ...
            <p className="text-gray-500 mt-2 text-center text-sm leading-relaxed">
              {isLogin ? t("auth.welcomeBack") : t("auth.protectInternet")}
            </p>
          </div>
